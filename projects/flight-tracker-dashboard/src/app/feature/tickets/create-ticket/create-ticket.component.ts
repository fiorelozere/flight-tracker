        ...
        <mat-card-content>
          ...
          >
            ...
            @if (form.get('outbound')?.hasError('required')) {
              <mat-error> Outbound city is required</mat-error>
            }
          </mat-form-field>
          ...
          @if (form.hasError('dateRangeInvalid')) {
            <mat-error> From date must be earlier than to date.</mat-error>
          }
          <mat-form-field
            appearance="outline"
            class="mx-2 my-4 w-full max-w-sm"
          >
            ...
            @if (form.get('seat_number')?.hasError('required')) {
              <mat-error> Seat number is required</mat-error>
            }
          </mat-form-field>
        </mat-card-content>
      </form>
...
export default class CreateTicketComponent {
  ...
  onSave() {
    ...
    this.createTicket({
      ...
      from_date: this.form.value.from_date
        ? new Date(this.form.value.from_date).toLocaleDateString()
        : null,
      to_date: this.form.value.to_date
        ? new Date(this.form.value.to_date).toLocaleDateString()
        : null,
    } as Ticket);
  }
   ...
   */
  createTicket = createEffect<Ticket>(
    pipe(
